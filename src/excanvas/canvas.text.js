...
;(function () {
  ...
  ctxt.lookupFamily = function (family) {
    ...
    for (i = 0; i < list.length; i++) if ((face = faces[list[i]])) return face
    return false
  }
  ...
  ctxt.getFace = function (family, weight, style) {
    ...
    if (face && face[weight] && face[weight][style]) return face[weight][style]
  ...
  proto.getComputedStyle = function (style) {
    ...
    // Compute the size
    if (typeof s === 'number' || s.indexOf('px') != -1) computedStyle.size = fontSize
    else if (s.indexOf('em') != -1) computedStyle.size = canvasFontSize * fontSize
    else if (s.indexOf('%') != -1) computedStyle.size = (canvasFontSize / 100) * fontSize
    else if (s.indexOf('pt') != -1) computedStyle.size = fontSize / 0.75
    else computedStyle.size = canvasFontSize
