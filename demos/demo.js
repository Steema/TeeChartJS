...
function changeTheme(chart, aTheme) {
  ...
  for (var i = 0; i < chart.series.count(); i++) {
    if (chart.series.items[i].pointer && chart.series.items[i].pointer.format) chart.series.items[i].pointer.format.stroke.fill = 'white'
  }
}
...
function changePalette(aChart, aPalette) {
  ...
  if (aChart && aChart instanceof Tee.Chart) target.push(aChart)
  else {
...
function changeBackground(aTheme) {
  if (aTheme === 'dark') changeBackgroundToDark()
...
function withTextAreas() {
  ...
  $('canvas').each(function () {
    if ($(this).siblings('div').children('textarea').length > 0) result = true
  })
...
$(function () {
  ...
  $(window).load(function () {
    if (textAreas) resizeAllWithTextAreas()
    else resizeAll()
  })
  ...
  if (textAreas) {
    ...
    $textareas.mousemove(function () {
      var $this = $(this)
      if ($this.data('resizing') && ($this.width() !== $this.data('width') || $this.height() !== $this.data('height'))) {
