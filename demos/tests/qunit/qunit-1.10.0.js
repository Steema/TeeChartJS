...
;(function (window) {
  ...
  function done() {
    ...
    var i,
      ...
      passed = config.stats ? config.stats.all - config.stats.bad : 0
    html = [
      'Tests completed in ',
      runtime,
      ' milliseconds.<br/>',
      ...
      config.stats ? config.stats.all : 0,
      "</span> passed, <span class='failed'>",
      config.stats ? config.stats.bad : 0,
      '</span> failed.'
    ].join('')
    ...
    if (config.altertitle && typeof document !== 'undefined' && document.title) {
      ...
      document.title = [config.stats ? (config.stats.bad ? '\u2716' : '\u2714') : '', document.title.replace(/^[\u2714\u2716] /i, '')].join(' ')
    }

    // clear own sessionStorage items if all tests passed
    if (config.reorder && defined.sessionStorage && config.stats && config.stats.bad === 0) {
    ...
    runLoggingCallbacks('done', QUnit, {
      failed: config.stats ? config.stats.bad : 0,
      passed: passed,
      total: config.stats ? config.stats.all : 0,
      runtime: runtime
    })
  }
