...
;(function () {
  ...
  function clearTable(table) {
    ...
    if (document.addEventListener) table.innerHTML = ''
  }
  ...
  Tee.Data.prototype.fillTable = function (id, result, header, totals) {
    ...
    if (typeof id === 'string') table = document.getElementById(id)
    ...
    function aggregate(result, value, measure) {
      switch (measure) {
        case 'high':
          if (result.count === 0 || value > result.value) result.value = value
          break
        case 'low':
          if (result.count === 0 || value < result.value) result.value = value
          break
    ...
    function finishAggregate(value) {
      ...
      else if (totals == 'average' && value.count > 0) value.value = value.value / value.count
    }
    ...
    if (header) {
      ...
      if (hasDim2) {
        ...
        if (res0dim) addHeader(r, res0dim.title).colSpan = resl
      } else {
      ...
      if (hasDim2) {
        ...
        if (result[0].masterdim) addHeader(r, result[0].masterdim.title)
        else addHeader(r, '')

        for (rest = 0; rest < resl; rest++) {
          res = result[rest]

          if (res.master) addHeader(r, res.master)
          else addHeader(r, res.dimension.title)
        }

        if (totals && resl > 1) addHeader(r, totals).className = 'totals'
      }
    }
    ...
    if (totals) {
      ...
      for (rest = 0; rest < resl; rest++) {
        ...
        for (var tv = 0; tv < res.values.length; tv++) aggregate(vtemp, res.values[tv].value, totals)
        ...
        for (tt = 0; tt < k.length; tt++) {
          ...
          if (sub instanceof Array) {
            for (var ttt = 0; ttt < sub.length; ttt++) {
              ...
              c.innerHTML = value
              if (typeof value === 'number') c.style.textAlign = 'right'
            }
          } else if (typeof sub === 'object') fillCells(r, sub)
          else {
  ...
  Tee.Data.prototype.MapLegend = function (legend, table) {
    ...
    function mapover() {
      ...
      for (t = 0; t < n.length; t++) {
        ;(id = n[t].getAttribute('id')), (v = USAMap.values[id])
    ...
    this.highlight = function (value, color) {
      ...
      for (; t < l; t++) {
        ...
        if (value) {
          if (t == 0) s.outline = value >= i.min && value < i.max ? color : ''
          else s.outline = value > i.min && value <= i.max ? color : ''
        } else s.outline = ''
      }
    }
  }
  ...
  Tee.DataEditor = function (editorframe, engine, chart) {
    ...
    this.checkDateDim = function (dim, select, divId) {
      ...
      if (divId) get(divId).style.display = da ? 'inline' : 'none'
      else sel.style.display = da ? 'inline' : 'none'
    ...
    function fillDimensions(select, addNull, filterDim, current) {
      ...
      for (var t = 0; t < d2.length; t++) {
        ...
        if (!filterDim || filterDim(d)) sel.add(new Option(d.title, dimensions.indexOf(d)))
      }
    ...
    function fillMetrics(id) {
      ...
      for (var t = 0; t < metrics.length; t++) sel.options.add(new Option(metrics[t].title, t))
      return sel
    }
    ...
    this.setSortOrder = function (value) {
      ...
      if (chart.sort.sortBy == '') this.doQuery()
      else {
    ...
    this.checkFilter = function () {
      ...
      if (p.options.length == 0) {
        fillDimensions('filterdim', false, function (d) {
          return !d.dontFilter
        })

        if (p.options.length > 0) this.setFilterDim(0)
      }
    }
    ...
    this.setFilterValues = function (d) {
      ...
      for (var t = 0; t < values.length; t++) {
        ...
        if (d.inSelected(d.datetime ? values[t] : d.get(values[t]))) option.selected = d.selectedInclude
    ...
    function fillArrays() {
      ...
      for (t = 0; t < d.length; t++) {
        ...
        for (tt = 0; tt < da.length; tt++) {
          ...
          for (ttt = 0; ttt < dim.metrics.length; ttt++) metrics.push(dim.metrics[ttt])
        }

        for (ttt = 0; ttt < d[t].metrics.length; ttt++) metrics.push(d[t].metrics[ttt])
      }
    }
    ...
    if (chart.sort.sortBy == 'values') get('sort').options[1].selected = true
    else if (chart.sort.sortBy == 'labels') get('sort').options[2].selected = true
    else get('sort').options[0].selected = true
    ...
    this.setFilterValue = function (value) {
      ...
      for (var t = 0; t < opt.length; t++) opt[t].style.fontWeight = 'normal'

      if (sel == -1) di.selected = null
      else {
        var o = opt[sel + 1]
        di.selected = o.text
    ...
    this.setDimension1 = function (index, dontexecute) {
      ...
      } else get('dimdate1').style.display = 'none'

      if (!dontexecute) this.doQuery()
    }
    ...
    this.setDimension2 = function (index, dontexecute) {
      ...
      } else get('dimdate2').style.display = 'none'
    ...
    this.setSeriesStyle = function (value) {
      ...
      get('smooth').style.display = !d || d == Tee.Line || d == Tee.Area || d == Tee.HorizArea ? 'inline' : 'none'
    ...
    function setSeriesStack(series, value) {
      if (series instanceof Tee.CustomBar || series instanceof Tee.CustomSeries) {
        ...
        if (series instanceof Tee.Area) series.format.transparency = series.stacked == 'no' ? 0.1 : 0
      }
    }
    ...
    function refreshScatter() {
      ...
      var p = chart.addSeries(new Tee.PointXY([])),
        po = p.pointer
      po.width = 6
      po.height = 6
      po.style = 'ellipse'
      ...
      if (get('viewtable').value == 'query') fillQueryResults()
    }
    ...
    function checkScatter() {
      ...
      if (p.options.length == 0) fillMetrics('scatter1').options[2].selected = true

      this.refresh()
    }

    function setSelect(sel, current) {
      var s = get(sel).options,
        t
      for (t = 0; t < s.length; t++) s[t].selected = s[t].value == current
    }
    ...
    */
      ...
      if (chart.animation && chart.animation.active) chart.animation.animate()
      else chart.draw()
    ...
    this.setStroke = function (value) {
      chart.series.each(function (s) {
        s.format.stroke.fill = value ? 'black' : ''
      })
      chart.draw()
    }
    ...
    this.setSmooth = function (value) {
      chart.series.each(function (s) {
        if (typeof s.smooth !== 'undefined') s.smooth = value ? 0.5 : 0
      })
      chart.draw()
    }
  }
